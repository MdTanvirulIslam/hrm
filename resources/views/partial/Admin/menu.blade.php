    ...
    <div class="navbar-wrapper">
        ...
        <div class="navbar-content">
            <ul class="dash-navbar">
                ...
                @else
                    @if (\Auth::user()->can('Manage User') || \Auth::user()->can('Manage Role') || \Auth::user()->can('Manage Employee Profile') || \Auth::user()->can('Manage Employee Last Login'))
                        <li class="dash-item dash-hasmenu">
                            <a href="#!" class="dash-link">
                ...
                @can('Manage Employee')
                    @if (\Auth::user()->type == 'employee')
                        ...
                        <li class="dash-item {{ Request::segment(1) == 'employee' ? 'active' : '' }}">
                            <a href="{{ route('employee.show', \Illuminate\Support\Facades\Crypt::encrypt($employee->id)) }}" class="dash-link">
                                <span class="dash-micon"><i class="ti ti-user"></i></span>
                ...
                {{-- BG/PG --}}
                <li class="dash-item {{ Request::segment(1) == 'bgpg' ? 'active' : '' }}">
                    <a href="{{ route('bgpg.index') }}" class="dash-link">
                        <span class="dash-micon"><i class="ti ti-shield-check"></i></span>
                        <span class="dash-mtext">{{ __('BG/PG') }}</span>
                    </a>
                </li>
                ...
                {{-- Payroll --}}
                @if (\Auth::user()->type == 'employee')
                    @if (\Auth::user()->can('View Own Salary') || \Auth::user()->can('View Own Payslip'))
                        <li class="dash-item dash-hasmenu {{ Request::segment(1) == 'setsalary' ? 'dash-trigger active' : '' }}">
                            <a href="#!" class="dash-link">
                                ...
                                <span class="dash-mtext">{{ __('Payroll') }}</span>
                                <span class="dash-arrow"><i data-feather="chevron-right"></i></span>
                            </a>
                            <ul class="dash-submenu">
                                @can('View Own Salary')
                                    <li class="dash-item {{ Request::segment(1) == 'setsalary' ? 'active' : '' }}">
                                        <a class="dash-link" href="{{ route('setsalary.show', \Auth::user()->id) }}">{{ __('Set Salary') }}</a>
                                    </li>
                                @endcan
                ...
                {{-- Timesheet --}}
                @if (\Auth::user()->can('Manage Attendance') || \Auth::user()->can('Manage Leave') || \Auth::user()->can('Manage TimeSheet'))
                    <li class="dash-item dash-hasmenu">
                        <a href="#!" class="dash-link">
                ...
                @if (\Auth::user()->can('Manage Indicator') || \Auth::user()->can('Manage Appraisal') || \Auth::user()->can('Manage Goal Tracking'))
                    <li class="dash-item dash-hasmenu">
                        <a href="#!" class="dash-link">
                            ...
                            <span class="dash-mtext">{{ __('Performance') }}</span>
                            <span class="dash-arrow"><i data-feather="chevron-right"></i></span>
                        </a>
                        <ul class="dash-submenu">
                ...
                {{-- Finance --}}
                @if (\Auth::user()->can('Manage Account List') || \Auth::user()->can('Manage Payee') || \Auth::user()->can('Manage Payer') || \Auth::user()->can('Manage Deposit') || \Auth::user()->can('Manage Expense') || \Auth::user()->can('Manage Transfer Balance') || \Auth::user()->can('View Balance Account List'))
                    <li class="dash-item dash-hasmenu">
                        <a href="#!" class="dash-link">
                ...
                {{-- Training --}}
                @if (\Auth::user()->can('Manage Trainer') || \Auth::user()->can('Manage Training'))
                    <li class="dash-item dash-hasmenu {{ Request::segment(1) == 'training' ? 'dash-trigger active' : '' }}">
                        <a href="#!" class="dash-link">
                            ...
                            <span class="dash-mtext">{{ __('Training') }}</span>
                            <span class="dash-arrow"><i data-feather="chevron-right"></i></span>
                        </a>
                        <ul class="dash-submenu">
                            @can('Manage Training')
                ...
                {{-- HR Admin Setup --}}
                @if (\Auth::user()->can('Manage Awards') || \Auth::user()->can('Manage Transfer') || \Auth::user()->can('Manage Resignation') || \Auth::user()->can('Manage Travels') || \Auth::user()->can('Manage Promotion') || \Auth::user()->can('Manage Complaint') || \Auth::user()->can('Manage Warning') || \Auth::user()->can('Manage Termination') || \Auth::user()->can('Manage Announcement') || \Auth::user()->can('Manage Holiday'))
                ...
                {{-- Recruitment --}}
                @if (\Auth::user()->can('Manage Job') || \Auth::user()->can('Manage Job Application') || \Auth::user()->can('Manage Job OnBoard') || \Auth::user()->can('Manage Custom Question') || \Auth::user()->can('Manage Interview Schedule') || \Auth::user()->can('Manage Career'))
                    <li class="dash-item dash-hasmenu {{ (Request::segment(1) == 'job' || Request::segment(1) == 'job-application') ? 'dash-trigger active' : '' }}">
                        <a href="#!" class="dash-link">
                            ...
                            <span class="dash-mtext">{{ __('Recruitment') }}</span>
                            <span class="dash-arrow"><i data-feather="chevron-right"></i></span>
                        </a>
                        <ul class="dash-submenu">
                            @can('Manage Job')
                ...
                @can('Manage Ticket')
                    <li class="dash-item {{ Request::segment(1) == 'ticket' ? 'active' : '' }}">
                        <a href="{{ route('ticket.index') }}" class="dash-link">
                            <span class="dash-micon"><i class="ti ti-ticket"></i></span>
                            <span class="dash-mtext">{{ __('Ticket') }}</span>
                        </a>
                    </li>
                @endcan
                ...
                @can('Manage Meeting')
                    <li class="dash-item {{ Request::segment(1) == 'meeting' ? 'active' : '' }}">
                        <a href="{{ route('meeting.index') }}" class="dash-link">
                            ...
                            <span class="dash-mtext">{{ __('Meeting') }}</span>
                        </a>
                    </li>
                @endcan
                ...
                @can('Manage Company Policy')
                    <li class="dash-item">
                        <a href="{{ route('company-policy.index') }}" class="dash-link">
                ...
                {{-- Plan Request (Super Admin Only) --}}
                {{-- @if (\Auth::user()->type == 'super admin')
                    <li class="dash-item">
                        <a href="{{ route('plan_request.index') }}" class="dash-link">
                            <span class="dash-micon"><i class="ti ti-arrow-down-right-circle"></i></span>
                ...
                {{-- Order (Super Admin Only) --}}
                @if (\Auth::user()->type == 'super admin')
                    @can('Manage Order')
                        <li class="dash-item">
                            <a href="{{ route('order.index') }}" class="dash-link {{ request()->is('orders*') ? 'active' : '' }}">
                ...
                {{-- HRM System Setup --}}
                @if (\Auth::user()->can('Manage Department') || \Auth::user()->can('Manage Designation') || \Auth::user()->can('Manage Document Type') || \Auth::user()->can('Manage Branch') || \Auth::user()->can('Manage Award Type') || \Auth::user()->can('Manage Termination Types') || \Auth::user()->can('Manage Payslip Type') || \Auth::user()->can('Manage Allowance Option') || \Auth::user()->can('Manage Loan Options') || \Auth::user()->can('Manage Deduction Options') || \Auth::user()->can('Manage Expense Type') || \Auth::user()->can('Manage Income Type') || \Auth::user()->can('Manage Payment Type') || \Auth::user()->can('Manage Leave Type') || \Auth::user()->can('Manage Training Type') || \Auth::user()->can('Manage Job Category') || \Auth::user()->can('Manage Job Stage'))
                    <li class="dash-item dash-hasmenu">
                        <a href="#!" class="dash-link">
                            <span class="dash-micon"><i class="ti ti-table"></i></span>
                            <span class="dash-mtext">{{ __('HRM System Setup') }}</span>
                            <span class="dash-arrow"><i data-feather="chevron-right"></i></span>
                        </a>
                        <ul class="dash-submenu">
                            @can('Manage Branch')
                            ...
                            @can('Manage Expense Type')
                                <li class="dash-item {{ request()->is('expensetype*') ? 'active' : '' }}">
                                    <a class="dash-link" href="{{ route('expensetype.index') }}">{{ __('Expense Type') }}</a>
                                </li>
                            @endcan
                            @can('Manage Income Type')
                                <li class="dash-item {{ request()->is('incometype*') ? 'active' : '' }}">
                                    <a class="dash-link" href="{{ route('incometype.index') }}">{{ __('Income Type') }}</a>
                                </li>
                            @endcan
                            @can('Manage Payment Type')
                                <li class="dash-item {{ request()->is('paymenttype*') ? 'active' : '' }}">
                                    <a class="dash-link" href="{{ route('paymenttype.index') }}">{{ __('Payment Type') }}</a>
                                </li>
                            @endcan
                ...
                {{-- System Setting --}}
                @if (\Auth::user()->can('Manage Company Settings') || \Auth::user()->can('Manage System Settings'))
                    <li class="dash-item">
