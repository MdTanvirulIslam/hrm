    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('Create Document'))
        {
            $validator = \Validator::make(
                $request->all(), [
                                   'name' => 'required',
                                   'documents' => 'required|max:2048000',
            ...
            $uploadedFile = $request->file('documents');
            $extension = $uploadedFile->getClientOriginalExtension();
            $mimeType = $uploadedFile->getMimeType();

            $allowedExtensions = ['doc', 'docx', 'jpg', 'jpeg', 'png', 'xls', 'xlsx', 'csv', 'pdf'];
            $allowedMimeTypes = [
                'application/msword',
            ...
            if (!in_array($extension, $allowedExtensions) || !in_array($mimeType, $allowedMimeTypes)) {
                return redirect()->back()->with('error', 'The document must be a valid file type.');
            }
            ...
            if(!empty($request->documents))
            {
                ...
                $dir = 'storage/uploads/documentUpload/';
                $image_path = $dir . $fileNameToStore;
                if (\File::exists($image_path)) {
                    \File::delete($image_path);
                }
                $request->documents->move($dir,$fileNameToStore);
                $url = '';
